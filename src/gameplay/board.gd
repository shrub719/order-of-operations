...
func advance_stage():
	...
	for x in range(board_size):
		for y in range(board_size):
			...
			var right = null if x == board_size - 1 else get_block_at(Vector2(x + 1, y))
			var up = null if y == 0 else get_block_at(Vector2(x, y - 1))
			var down = null if y == board_size - 1 else get_block_at(Vector2(x, y + 1))
